  ...
  return (
    <motion.nav
      ...
      className={`fixed w-full top-0 z-50 transition-all duration-500 ${scrolled
        ? "bg-white/95 dark:bg-gray-950/95 border-b border-gray-100 dark:border-gray-800 shadow-lg backdrop-blur-md"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-3 md:py-4 flex justify-between items-center">
        ...
        {/* Desktop Menu */}
        <ul className={`hidden md:flex gap-2 items-center transition-colors duration-300 ${scrolled ? "text-gray-800 dark:text-gray-200" : "text-white"
          }`}>
          {NAV_ITEMS.map((item) =>
            item.dropdown ? (
              <li
              ...
              >
                <button className="flex items-center gap-1 px-4 py-2 rounded-lg transition-all duration-300 relative group">
                  <span className={`relative z-10 transition-colors ${hoveredItem === item.name
                    ? 'text-white'
                    : scrolled
                      ? 'text-gray-800'
                      : 'text-white'
                  ...
                  <motion.span
                    ...
                    className={`relative z-10 transition-colors ${hoveredItem === item.name
                      ? 'text-white'
                      : scrolled
                        ? 'text-gray-800'
                        : 'text-white'
                      ...
                      {item.dropdown.map((subItem, idx) => (
                        ...
                        >
                          <Link
                            ...
                            className="block py-3 px-5 text-gray-800 dark:text-gray-200 hover:bg-gradient-to-r hover:from-primary-600 hover:to-secondary-600 hover:text-white transition-all duration-200"
                          >
                            {subItem}
                          </Link>
                        </motion.li>
              ...
              <li key={item.name}>
                ...
                >
                  ...
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-r from-primary-600 to-secondary-600 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                    initial={false}
                  />
                  <span className={`relative z-10 transition-colors duration-300 ${location.pathname === item.path
                    ? scrolled
        ...
        <div className="md:hidden flex items-center gap-2">
          ...
          <motion.button
            ...
            className={`text-2xl focus:outline-none p-2 transition-colors duration-300 ${scrolled ? "text-gray-800 dark:text-gray-200" : "text-white"
              }`}
            whileTap={{ scale: 0.9 }}
          >
            <motion.span
              ...
              item.dropdown ? (
                <li key={item.name} className="border-b border-gray-200 dark:border-gray-700">
                  <button
                    ...
                    className="w-full text-left py-3 px-4 bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 hover:bg-primary-600 hover:text-white transition"
                  >
                    {item.name} ▼
                  </button>
                      ...
                      >
                        {item.dropdown.map((subItem) => (
                          <li key={subItem}>
                            <Link
                              to={getBusinessPath(subItem)}
                              className="block py-2 px-6 text-gray-800 dark:text-gray-200 hover:bg-primary-600 hover:text-white transition"
                ...
                <li key={item.name} className="border-b border-gray-200 dark:border-gray-700">
                  <Link
                    to={item.path}
                    className="block py-3 px-4 text-gray-800 dark:text-gray-200 hover:bg-primary-600 hover:text-white transition"
                    onClick={() => setMenuOpen(false)}
                  >
                    {item.name}
                  </Link>
